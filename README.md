...
**THIS ROM IS STRICTLY FOR THE SM-T560 (Wi-Fi ONLY) MODEL. DO NOT FLASH THIS ON THE SM-T561 (3G/Cellular).**

### Disclaimer & Credits:

**I take ZERO credit for the original ROM base, the OS framework, or the custom kernel.** 
99% of the heavy lifting and critical development was done by the incredibly talented original ViperOS porter for this device, **gitanillo87**, and the custom kernel developer **A404_**. 
...
* **Master Ramdisk Optimization:** The kernel boot image has been surgically stripped of 1000+ lines of generic AOSP, Qualcomm, and HTC code. I/O block queues are forced to the `deadline` scheduler natively, eMMC entropy generation is disabled, and CPUSets strictly pin background tasks to a single core while giving the UI access to all 4 cores.
* **Massive Memory Savings:** Reduced the ROM zip size by **~100MB** and gutted background OS RAM usage. The tablet now idles around **425MB - 550MB of RAM** (even with the Google Play Store installed)!
...
* **UI & Display Scaling Fixes:** 
  * The `build.prop` has been corrected with the exact physical dimensions of the 9.6-inch panel (Width: 207mm, Height: 129mm). LCD Density is set to `180` to properly scale the UI for a true tablet experience.
  * Replaced the laggy default AOSP keyboard with a custom, lightweight keyboard that is much faster, better-scaled, and more comfortable to type on.
  * Fixed the Multi-User soft-reboot by completely disabling the Android System Profile engine (`fw.max_users=1`).
* **Advanced `build.prop` Tweaks:**
  * Fixed the screen-off Wi-Fi disconnect bug by adjusting power collapse and sleep policy states. [NEEDS MORE TESTING]
...
* **Modern Web Rendering & Aesthetics:** Updated the core Android System WebView to v117. Modern websites no longer crash, SSL certificates work, and layout issues are fixed. The default boot animation was replaced with the Google Pixel Gemini animation, for better aesthetics.

---

## 🐛 Known Bugs

While this ROM is highly stable for daily use, there are some bugs carried over from the source base:
1. **Random Restarts Under High Load:** Occasionally, putting the CPU under maximum load may cause a random soft reboot. This is caused by the custom Overclocked Kernel (by A404_) and is beyond my ability to patch. However, extended usage seems to miraculously reduce its frequency.
~~2. **Wi-Fi Notification Ping (Screen Off):** Sometimes, when the screen turns off, the tablet will constantly play a notification sound reporting "Wi-Fi connected but no internet," which disappears when the screen is turned on.~~
...
* **Language Support:** **English Only.** All other system languages, fallback fonts, and hyphenation dictionaries were stripped to save memory. Though installing a GApps ZIP re-adds them.
...
* **System Apps Removed:** To keep the OS RAM footprint tiny, `PrintSpooler`, `ContactsProvider`, `CalendarProvider`, and `Live Wallpapers` have been permanently removed. Some system apps might have been useful to specific users.
